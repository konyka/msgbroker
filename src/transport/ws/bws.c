...
static char *mb_ws_find_header (const char *req, const char *name)
{
...
static int mb_bws_do_handshake (int fd)
{
    ...
    key = mb_ws_find_header (req, "Sec-WebSocket-Key:");
    if (!key)
        return -1;
    ...
    if (key_len + sizeof (mb_ws_accept_key) - 1 > sizeof (hash_input))
        return -1;
...
int mb_bws_create (struct mb_ep *ep)
{
    ...
    self = (struct mb_bws *) mb_alloc (sizeof (struct mb_bws));
    if (!self) {
        close (fd);
        return -ENOMEM;
    }
    ...
    mb_thread_start (&self->accept_thread, mb_bws_accept_loop, self);
