...
/*  PUB POLLOUT must clear under TCP backpressure and return after drain. */
static void test_pub_poll_polout_after_backpressure (void)
{
    int pub, sub;
    int rc;
    int i;
    ...
    rc = mb_bind (pub, "tcp://127.0.0.1:18896");
    assert (rc >= 0);
    usleep (50000);
    rc = mb_connect (sub, "tcp://127.0.0.1:18896");
    assert (rc >= 0);
    usleep (100000);
    ...
    for (i = 0; i < 512; ++i) {
        rc = mb_send (pub, buf, sizeof (buf), MB_DONTWAIT);
        if (rc < 0) {
            assert (mb_errno () == EAGAIN);
            hit_eagain = 1;
            break;
...

    {
        int woke = 0;
        for (i = 0; i < 100; ++i) {
            while (mb_recv (sub, rbuf, sizeof (rbuf), MB_DONTWAIT) > 0)
                ;
            fds[0].revents = 0;
            rc = mb_poll (fds, 1, 50);
            if (rc >= 1 && (fds[0].revents & MB_POLLOUT)) {
    ...
    rc = mb_send (pub, "OK", 2, MB_DONTWAIT);
    assert (rc == 2);
